...
### 2.0.6 我的成长页·用户信息区实现（本地版·当前 v2.0）
> 范围：仅前端本地存储（`wx.setStorageSync`），**不接云、无退出登录**（账户体系云版已推迟，见 MEMORY §3.14/#36）。本特性为云版（§2.0.7）的"推迟期派生实现"——先把"设置头像/昵称"这件不依赖云的事做掉。

**EARS 需求**
- **Ubiquitous**：系统始终在成长页（stats）顶部用户信息区展示当前用户的头像与昵称，数据源为本地 `wx.setStorageSync('userProfile')`。
- **Event-driven**：
  - When 用户点击头像（`open-type="chooseAvatar"` 按钮），系统 shall 调起微信原生头像选择器，用户选定后更新头像。
  - When 用户编辑昵称（`input type="nickname"`）并失焦/确认，系统 shall 将昵称写入本地 `userProfile`。
- **Unwanted**：
  - If 微信安全检测未通过（基础库 2.24.4+），系统 shall 不更新头像/昵称（回调不触发），保持原值。
  - If 基础库 < 2.21.2（`chooseAvatar` / `input type=nickname` 不可用），系统 shall 退化为默认头像 + 普通 `input`（方案 A 不额外实现该降级，要求基础库 ≥ 2.21.2，见 MEMORY §3.15）。
- **State-driven**：While `userProfile` 为空（首次/未设置），系统 shall 显示默认占位头像（🌱/松鼠占位）+ 默认昵称，且**不强制引导**。
- **Optional**：Where 用户从未设置昵称，系统 shall 显示默认昵称（非阻断）。

**数据模型（新增，仅本地）**
```js
// utils/storage.js 封装；持久化键 'userProfile'
{ avatarUrl: 'wxfile://...',   // saveFile 持久化后的本地路径（非临时路径）
  nickname: '果果的好朋友' }
```

**任务拆分（T1–T10，架构师交付 → 工程师执行）**
| # | 任务 | 关键实现 | 验收点 |
|---|------|----------|--------|
| T1 | WXML 结构 | `stats.wxml` **移除本页 `page-header`**（用户卡作页面主体），在 `screen` 顶部、`streak-section` 之前**增量插入** `user-card`（头像 `<button open-type="chooseAvatar">` + 昵称 `<input type="nickname">` + 右下角 ✎ 铅笔角标）；打卡/任务页 `page-header` **保留** | 我的成长页仅用户卡、另两页标题不变 |
| T2 | WXSS 样式 | `stats.wxss` 加 `.user-card` 等，对齐 design-spec §8.1 Token | 1:1 对齐视觉稿 |
| T3 | 存储适配 | `utils/storage.js` 增 `getProfile()/setProfile()`，复用既有适配层 | 读写封装到位 |
| T4 | 数据加载 | `stats.js` `onLoad` 读 `userProfile` 渲染；为空 → 占位默认头像/昵称 | 首启显示占位 |
| T5 | 头像选择 | `bind:chooseavatar` 取 `e.detail.avatarUrl`（临时路径）→ `wx.getFileSystemManager().saveFile()` 持久化到本地用户目录 → 写 `userProfile.avatarUrl` | 选完即换、重启仍在 |
| T6 | 昵称编辑 | `input type="nickname"` `bindblur`/`bindconfirm` 取值 → `setProfile` 写本地 → `setData` 刷新 | 失焦即存、刷新可见 |
| T7 | 头像兜底 | `saveFile` 失败 → 保留上次有效头像，不写入损坏路径 | 失败不崩 |
| T8 | 安全检测对齐 | 无额外代码（未过检则不触发回调=自然保持原值）；回归验收时说明 | 无需改动 |
| T9 | 回归与验收 | `miniprogram-ci compile` ✅ + 下方统计块零变化 + 真机验证（见验收标准） | 三项达标 |
| T10 | TabBar 顺序与文案 | 修改 `app.json` `tabBar.list`：顺序改为「打卡 / 任务 / 我的成长」；第三项 `text` 改为「我的成长」，图标路径按 v2.0 设计稿同步更新 | 与 design-spec §3.9 / 原型一致 |

**微信官方约束核验（#17 / QC7）**
- `chooseAvatar`（`open-type` 按钮）与 `input type="nickname"` 均自**基础库 2.21.2** 起支持；头像必须由 `<button open-type="chooseAvatar">` 包裹（view/div 不能触发原生选择器）。
- `avatarUrl` 是**临时文件路径**，程序退出/缓存回收后失效，必须 `saveFile` 持久化到本地用户目录。
- 安全检测（2.24.4+）：头像 `mediaCheckAsync`、昵称 `msgSecCheck`，**未过不触发回调/清空输入**；我们纯前端、零后端、零网络请求，与"不接云"完全不冲突（检测发生在用户选择/输入当下，由微信服务端透明完成）。
- 真机约束：DevTools 模拟 `input` 不还原真机行为，原生组件必须**真机体验版**验证（QC22/#34）。

**验收标准**
1. `miniprogram-ci compile` ✅。
2. 真机扫码：点头像 → 微信原生选择器 → 选完头像替换；编辑昵称失焦 → 本地保存并刷新。
3. 杀进程重进：头像/昵称仍在（本地持久化生效）。
4. 下方统计块（连续天数/完成率/月历/徽章）与改前**逐像素一致**（零回归，QC5/QC6）。
5. 隐私：本期**不加任何隐私文案**（推迟到下一版「小程序使用引导规划」），平台《隐私保护指引》不额外声明（与 v1.0 一致，#41）。

### 2.0.7 我的成长页·用户信息区实现（云版·推迟，账户体系）
> 以下为账户体系落地时的形态；当前 v2.0 本地版（§2.0.6）不依赖云，本小节待 env-id 到位后启用。

- **入口**：合并进底部 Tab「我的成长」页内，用户头像/昵称区域就地点改；不新增独立「我的」页。
...
### 2.0.11 验收与边界（对齐 PRD 2.0.8 / 2.0.9）
- 跨设备同步、数据隔离、迁移完整、弱网降级、隐私三项声明——同 PRD 验收标准。
- 「退出登录」：清空本地缓存态，云数据保留（下次登录恢复）；入口位于「我的成长」页底部。**本项属于账户体系（云版），当前本地版（§2.0.6）无退出登录，见 MEMORY §3.14/#36。**
...
| v2.0 | 2026-07-19 | Sophia (AI) + 用户确认 | 成长页用户信息区本地版研发设计方案（§2.0.6：EARS + T1–T10 + 微信官方约束核验 + 验收标准；云版推迟为 §2.0.7，后续顺延 §2.0.8–§2.0.11；TabBar 顺序/文案 + 我的成长页移除 page-header 同步纳入 T10/T1） |
