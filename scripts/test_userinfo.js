...
test('T-C06', '边界', '未来日期 → 无论是否有任务/完成记录，都保持白底、无补字', () => {
  resetStore();
  _mockStore.state.tasks = [{ id: 1, is_deleted: false }];
  _mockStore.state.checkinsByDate = { '2026-07-19': { 1: { done: true, backfill: true } } };
  _page._buildCalGrid();
  // 沙箱当前日期为 2026-07-20，取第 21 天（未来日，index=20，firstDay=0）
  const futureDay = _page.data.calDays[20];
  if (!futureDay || futureDay.day !== 21) throw new Error('未定位到未来日格');
  eq(futureDay.color, '', '未来日不应着色');
  eq(futureDay.backfill, false, '未来日不应带补字');
});
test('T-C07', '正常', '口径与打卡页一致：5 个活跃任务完成 3 个 → 橙色（非绿色）', () => {
  resetStore();
  _mockStore.state.tasks = [
    { id: 1, is_deleted: false, repeat_type: 'daily' },
    { id: 2, is_deleted: false, repeat_type: 'daily' },
    { id: 3, is_deleted: false, repeat_type: 'daily' },
    { id: 4, is_deleted: false, repeat_type: 'daily' },
    { id: 5, is_deleted: false, repeat_type: 'daily' },
  ];
  _mockStore.state.checkinsByDate = { '2026-07-19': { 1: { done: true }, 2: { done: true }, 3: { done: true } } };
  _page._buildCalGrid();
  const c = firstCalDay();
  eq(c.color, 'orange', '5 活跃完成 3 应标橙色（部分完成），不能误标绿色');
  eq(c.backfill, false, '非全完成无补字');
});
test('T-C08', '正常', 'repeat_type 过滤：工作日不算周末任务，周末任务完成记录不污染工作日颜色', () => {
  resetStore();
  _mockStore.state.tasks = [
    { id: 1, is_deleted: false, repeat_type: 'daily' },
    { id: 2, is_deleted: false, repeat_type: 'weekend' },
  ];
  _mockStore.state.checkinsByDate = { '2026-07-19': { 1: { done: false }, 2: { done: true } } };
  _page._buildCalGrid();
  const c = firstCalDay(); // 7月1日是周三，仅 task1 活跃，且未完成 → 黄色
  eq(c.color, 'yellow', '工作日不应把周末任务的完成记录算进当天');
  eq(c.backfill, false, '未完成无补字');
});
...
test('T-W04', '结构', '未来日期白底 guard：stats.js 在着色逻辑前计算 isFuture，且 backfill 用 let 可重置', () => {
  const _js = fs.readFileSync(path.join(__dirname, '..', 'miniprogram', 'pages', 'stats', 'stats.js'), 'utf-8');
  if (!_js.includes('const isFuture = date > todayDate')) throw new Error('未在着色前计算 isFuture');
  if (!_js.includes('const isToday = iso === todayIso')) throw new Error('未在着色前计算 isToday');
  if (!_js.includes('!isFuture')) throw new Error('未来日期白底 guard 缺失');
  if (!_js.includes('let backfill = false')) throw new Error('backfill 未用 let 初始化，无法对未来日重置');
});
