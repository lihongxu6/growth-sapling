...
- **灰阶**：遮罩透明度取设计稿当前值 `rgba(0,0,0,.45)`（待确认项 §10-b 是否降到 .30）。
...
3. **概念步推进**：G3–G6 均在成长页，按"看一眼点下一步"推进（不强制操作目标），`advance()` 仅改 `step` + 重渲染本页 coach-mark。
...
1. **【首次启动默认 4 任务 vs G2「只显示孩子加的 1 个任务」】**（影响 G1/G2 演示态）
   设计稿 G2 叙事为"孩子只加了 1 个任务"。但 `constants.DEFAULT_TASKS` 真实预置 **4 个每日任务**。建议：**保留 4 默认任务**（不动产品播种行为，风险最低），G1 文案改为"添加你的第一个习惯吧～"、G2 高亮首个 `.task-card`。若产品坚持"空白起步 onboarding"，则需引导模式下抑制默认播种（改动更大，需评估）。
2. **【遮罩透明度】** 设计稿当前 `.45`（已随稿 commit）；MEMORY §2 曾记评审要把引导遮罩降到 `.30` 提升非高亮区可见度。取哪个？（建议 `.45`，与设计稿一致、对比清晰。）
3. **【概念步（G3–G6）推进条件】** 是"看一眼即可点下一步"，还是"必须点一下目标元素才解锁下一步"？建议前者（孩子认知负荷低、流程顺畅），与"概念性看图不强求操作"一致（§3.30）。
