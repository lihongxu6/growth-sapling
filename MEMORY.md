...
36. 🔄 **云开发成本分析与策略决策**（本会话）：用户确认 MVP 已上线，询问云开发免费额度。
经查官方计费文档确认：① 新用户免费 6 个月（前提：小程序未上线）；② 小程序已上线后，免费环境有效期缩短为上线后第 15 天；③ 基础套餐 19.9 元/月（5 折，原价 39）含 20 万次调用/2GB 存储/5GB CDN/QPS500；④ 活动截止 2026-12-31。
**影响**：本项目 MVP 已上线 → 若现在开通云开发，免费期仅 15 天。
**用户决策**：希望想办法延长免费期。
**推荐方案（开发态优先策略）**：Phase 1 先用 Mock/fallback 写完所有云相关代码（不开云环境，免费）；Phase 2 最后才开通云环境，用 15 天窗口完成真云联调和提审；Phase 3 第 14 天决策是否付费 19.9 元/月。用户待确认是否按此策略推进。
