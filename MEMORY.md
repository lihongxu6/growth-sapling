...
- 🔜 **v2.1 用户引导功能 · 阶段三设计稿进行中（2026-07-21 本会话启动，v2.0.1 审核通过触发）**：设计师角色产出高保真视觉稿 `ui-mockup-v2.1-guide.html`（Route A，6 帧：引导Step1欢迎/Step3打卡高亮/Step7改头像+G3授权弹窗/完成庆祝/操作手册单页/我的成长页手册入口增量）。设计决策：① 真实松鼠IP拟人(`avatar-144-transparent.png`，弃用emoji占位，PRD G4 Optional)；② 弱引导覆盖层(绿光圈1.4s脉冲+`rgba(0,0,0,0.45)`轻遮罩、`pointer-events:none`不锁屏)；③ G3 采用微信官方 `wx.requirePrivacyAuthorize` 系统弹窗样式，文案与G2同源；④ G4 成长页 user-card 下方**增量新增**「📖 操作手册」入口(虚线绿框+「新增」标)→单页纯静态图文手册，松鼠拟人贯穿6步。严格守 §3.10(不动物上线块)/§3.9(TabBar顺序)/design-spec Token。⚠️ 创作产物按 §3.2 两步确认：**稿已出未 commit，等用户评审点头才 commit+push**。评审通过→技术方案→研发。
...
44. ✅ **v2.0.1 审核通过 + v2.1 阶段三设计稿启动（2026-07-21，用户确认）**：用户告知 v2.0.1 审核已通过，并指令「直接以设计师角色做设计稿」。Sophia 切设计师/交互设计师角色，基于 v2.1 PRD(G1–G4)+design-spec，产出高保真视觉稿 `ui-mockup-v2.1-guide.html`（6 帧 + 设计说明，Route A，Token 严格对齐）。核心决策：真实松鼠IP拟人 / 弱引导覆盖层(不锁屏) / G3 官方授权弹窗 / G4 成长页增量新增手册入口。🔜 **待用户评审确认**（§3.2 两步确认：稿未 commit，确认后才 commit+push）→ 评审通过进技术方案→研发。发布状态(v2.0.1 是否点发布)亦待用户告知。
