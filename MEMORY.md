...
- **默认昵称与头像显示（2026-07-19 用户真机反馈）**：用户真机截图显示当前默认昵称为 "设置昵称" 且头像显示不全；用户明确要求按原型改为 **"果果的好朋友"** 作为默认昵称。头像显示不全根因：`<image>` 使用 `mode="aspectFit"` 导致内容未填满圆形头像区；修复方案：改用 `mode="aspectFill"` 使松鼠头像完整填充圆形，同时让 ✎ 编辑角标正确落在头像上。待确认：默认昵称是（A）作为预填充值并自动持久化到 storage，还是（B）仅预显示但首次不写入 storage、等用户编辑时再存。**已决策 A 并落地（2026-07-19 用户确认 via q-0）**：首次无昵称时预填「果果的好朋友」并自动持久化到 storage（profile.nickname='果果的好朋友'）。头像修复 `mode="aspectFit"` → `aspectFill` 已改 `stats.wxml`；默认昵称逻辑已改 `stats.js` `_loadProfile`；重新 compile ✅ + 重新出 preview 二维码 ✅；代码提交 `f5b0ad4`。自测脚本 `scripts/test_userinfo.js` T-U04 已同步为「首次默认果果的好朋友并持久化」，复跑 12/12 ✅。详见 `docs/测试验收报告-用户信息区v2本地版.md` 第七节。
