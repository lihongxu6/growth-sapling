...
20. 🔜 上线流程推进（2026-07-18）：v2.0 技术方案已写入 `tech-spec-成长小树苗.md`（2ee6318）。v1.0 开发版已重新上传（137KB，含未来日期文案修复）+ 预览二维码 `.preview-qr.png` 重新生成。下一步**待用户在微信公众平台控制台操作**：① 基础信息(名称=成长小树苗/松鼠头像/简介按攻略填) → ② 服务类目=工具-备忘录(主)+日历(备) → ③ 隐私指引：v1.0 无账户、无隐私接口调用，核实代码后按"未采集隐私"或最小声明 → ④ 版本管理→开发版 v1.0.0→提交审核(功能页加 3 页截图+30s 录屏+备注) → ⑤ 审核通过后**小程序备案**(ICP，个人:身份证+人脸核身+24h 验 12381 短信) → ⑥ 备案通过点发布。详见 `微信小程序提审攻略-2026.md`。
