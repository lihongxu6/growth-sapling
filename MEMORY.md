...
32. 🎨 **v2.0 开屏页设计稿已修订**（2026-07-19）：已基于 H5 开屏动画产出小程序高保真设计稿 `ui-mockup-v2-splash.html`，沿用 🌱 +「成长小树苗」+「陪你慢慢长大」、600ms splashPop 入场、1.5s 停留/点击跳过。按用户反馈**已移除底部「点击任意位置跳过」提示语**，避免分散注意力。待用户最终确认后进入代码实现：创建 `pages/splash` 三件套 + `app.json` 注册 + `app.js` 跳转控制。
