...
42. 🔜 **下个版本 = 用户引导功能（2026-07-20 用户确认）**：与 §3.15 决策一致——首次轻引导 + 隐私/透明提示均推迟到「下一版小程序使用引导规划」，从界面设计整体做。流程（用户确认）：**需求框架先讨论确认 → 再落到 `prd-成长小树苗.md`（living PRD，EARS 新版本章节）**。**状态（2026-07-20 收尾后）**：已进入**阶段一·需求规划**，框架草稿 `docs/需求框架-用户引导功能.md`（v0.1/待确认）已出，含 G1–G5 功能候选 + 范围外 + 3 项关键假设（受众/形态/隐私节点）+ 验收草案；待用户讨论确认 → 写 PRD → 设计稿 → 技术方案 → 研发。启动条件仍记：2.0.1 审核结果 + 《隐私保护指引》声明审核通过。关联 #57、§3.8（不堆独立页）、§3.10（不动物上线内容）、§3.15。
