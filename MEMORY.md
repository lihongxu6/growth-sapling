...
28. 🔄 H5 拉齐 + 国内托管（2026-07-18，范围已明确）：背景——H5 放个人简历作项目展示；用户不喜简历放二维码故用链接；但 **GitHub Pages 在大陆常无法打开**，H5 链接国内常失效，需改国内可访问托管（建议 **Gitee Pages 镜像**：免费、国内可访问、可自动同步 GitHub；注意 Gitee 需实名/企业认证才能开 Pages）。用户已确认：H5 与小程序技术栈不同、代码不复用，**分开维护即可**（不强行统一代码）。Sophia 评估：① 拉齐窗口=现在（pre-account），一次性把 H5 对齐到小程序 v1.x（含已修复项）后冻结为参考版，不追求持续同步；② 本地存储对简历 demo 反是优势（开箱即用、无需后端/账号）。下一步：① H5 vs 小程序差异盘点确认工作量；② 解决 H5 国内托管（Gitee Pages 镜像 or 国内静态托管/OSS+CDN）；③ 排期：建议 v2.0 研发前作独立小任务，或并入 v2.0。待用户确认排期与托管方案。
