...
42. 🔜 **下个版本 = 用户引导功能（2026-07-20 用户确认）**：与 §3.15 决策一致——首次轻引导 + 隐私/透明提示均推迟到「下一版小程序使用引导规划」，从界面设计整体做。启动条件：① 2.0.1 审核结果回来；② 《隐私保护指引》声明审核通过。建议新会话以「需求规划/设计」阶段切入，先出引导功能方案（首次进入引导页/分步引导 + 隐私授权说明文案），评审通过再研发。关联 #57（隐私声明）、§3.15（局部引导决策）。
