...
### 3.38 v2.1 操作手册设计倾向（2026-07-23 用户反馈，待确认落地）
- **内容**：操作手册内容倾向**复用 6 步引导的相同视觉与文案**（与首次引导保持一致、单源、简单高效），但**去掉浮层遮罩/skip/next**，改以干净卡片/插图形式呈现——引导浮层是叠加真实页的 coach-mark，手册是独立参考，不应带遮罩与按钮。核心操作 = 添加任务 / 打卡 / 看日历 / 看徽章 / 改头像。手册形态建议 = 底部弹层(sheet) 内 6 张干净卡片 + 「再走一遍引导」入口，不新增 TabBar（守 §3.9）。
- **入口**：`.manual-entry` 当前在 mockup 里是带「新增」角标的大色块，用户认为**太显眼**——操作手册是低频「备用」功能（用户走一遍首次引导即会操作），不应抢占注意力。**应改为小入口**：小图标(📖) + 小文字「操作手册」的链接 / 角落小按钮，置于成长页底部或角落，去掉「新增」角标。
- **纪律关联**：与 §3.9 TabBar 维持 3 项一致；与 §3.20 引导功能决策一致（手册是引导的「可回看」延伸）；与 §3.36/#71「基于真实源码/实测」一致。
