...
41. 🔜 **日历配色修复（方案 B + 未来日白底 + 活跃任务口径对齐，2026-07-20）**：用户真机发现「有任务0完成」日显示白底，已按方案 B 修复（黄/橙/绿+补字）；后续复核又发现未来日期不应着色，已追加「未来日期统一白底」逻辑；再后续发现 7月13日 日历绿但点进去还有未完成，已修复为「日历分母/分子与打卡页当天活跃任务口径一致」。25/25 自测 ✅、compile ✅、preview 重出 ✅、已 commit+push。`scripts/test_home.js` 已加（补卡完成庆祝弹窗 4/4 ✅，见 §3.19 / #61）。⚠️ 仍需用户：① ✅ 已 `upload` 新版本 **2.0.1**（144304 bytes，robot=1，2026-07-20 23:36，含方案B配色/未来日白底/活跃任务口径对齐+补卡完成庆祝弹窗；build.js 版本号已升 2.0.1+描述更新并 commit `d5e7c7f`）；② 🔴 提交审核已触发平台警告：检测到隐私接口 **Album**（相册），根因 `wx.chooseImage`；用户必须点「取消」，先在微信公众平台《隐私保护指引》声明相册(scope.album)+摄像头(scope.camera)，再重新提交审核（否则发布后相关权限被回收）；③ 🔜 代码侧补 `wx.requirePrivacyAuthorize` 隐私授权弹窗（基础库 2.32.3+ enforcement 设备需要）。详见 §3.15 第八/九/十轮 + §3.19 + #57 + #58 + #59 + #60 + #61 + 验收报告第十五/十六/十七/十八节。
