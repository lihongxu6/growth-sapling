...
- **设计师角色实例（2026-07-19）**：用户切 Sophia 回设计师角色，清理 `ui-mockup-v2-userinfo.html` 中残留的「退出登录」（标题/CSS/HTML/图例），并同步 `design-spec`（§3.12/§4.1）、`PRD`（§2.0.3 功能清单）、`tech-spec`（§2.0.10 注）对「退出登录」的描述，使所有交付物一致指向「当前本地版无退出登录，退出登录随账户体系（云版）推迟」。
