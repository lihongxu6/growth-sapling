...
32. 🎨 **v2.0 开屏页设计稿已出**（2026-07-19）：已基于 H5 开屏动画产出小程序高保真设计稿 `ui-mockup-v2-splash.html`，沿用 🌱 +「成长小树苗」+「陪你慢慢长大」、600ms splashPop 入场、1.5s 停留/点击跳过。待用户确认后进入代码实现：创建 `pages/splash` 三件套 + `app.json` 注册 + `app.js` 跳转控制。
