...
- **合并目标页（待用户拍板）**：用户信息(头像/昵称)+退出登录合并进某个现有 Tab 页（不新增「我的」）。原 Sophia 倾向成长页；**2026-07-18 用户新提放「任务页」**（直觉：任务页管"我要做哪些任务"，与"我是谁"同属个人管理）。Sophia 判断**推荐成长页**：成长页是"个人成果/数据看板"基调，顶部身份区语义最顺（"这是谁的成果"），退出登录等个人操作放"个人看板"底部更自然；任务页是纯任务 CRUD 操作页，塞身份区会稀释"任务管理"专注度。→ 等用户拍板目标页后，落地设计稿 + PRD/tech-spec v2.0 章节（删独立「我的」页、改入口与交互为「目标页就地编辑」）。
...
22. 🔜 用户信息页设计调整（2026-07-18 用户反馈 + 已细化）：用户质疑 v2.0 原「我的」独立页——微信登录不应让用户编辑资料、内容单薄、多开页增负担；决定合并进现有 Tab 页（不新增「我的」），TabBar 维持 3 项。**头像获取=方案 A 已定**：首次进入轻引导一次(`chooseAvatar`+`<input nickname>`，微信原生浮层点一下即填)，之后存云端 `users` 表，平时在目标页就地点改，不跳转独立编辑页（不做灰色默认头像方案 B）。**目标页待用户拍板**：用户新提放「任务页」（直觉：任务页管"我要做哪些任务"与"我是谁"同属个人管理）；Sophia 推荐「成长页」（语义：成长页=个人成果看板，顶部身份区"这是谁的成果"最顺；任务页=纯 CRUD 操作页，塞身份区稀释专注度）。技术澄清：小程序无法静默取真实微信头像昵称(`getUserProfile`已废弃)，必须用户主动一次。→ **待用户拍板目标页后**：改设计稿(目标页加用户信息区+底部退出登录)、同步改 PRD/tech-spec v2.0 章节(删独立「我的」页、改入口与交互)。原 `user-profile-design.html` 独立页稿作废，需重做。
