...
28. ✅ H5 拉齐=方案 A（2026-07-18 用户定）：现在把 H5 对齐到**最新 v2.0 设计稿**（非仅 v1.x），但 H5 仍本地存储、无注册/账户——只提供「修改用户名」(本地昵称)，不接云账户。故**无需给 H5 单独出设计稿**，直接复用 v2.0 我的成长页设计意图（用户信息区+修改用户名+退出登录+TabBar 重排 打卡/任务/我的成长）。范围：① 补 v1.0 MVP 已修项(如未来日期 banner)使 H5 不再旧；② 套用 v2.0 设计到 H5（用户信息区+改用户名+退出+TabBar 重排），头像/昵称走本地存储。**核查结论**：index.html 自包含 51KB、无外部资源→「慢」=GitHub Pages 托管、非代码，A 修不了「慢」，仍待 Gitee 国内托管(暂缓)。实施步骤：① 先做 H5 差异盘点列出改动清单；② 用户确认；③ 改 index.html；④ 渲染复核。下一步：执行 H5 差异盘点。
