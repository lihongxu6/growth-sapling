...
- G4「看日历」落点**待确认**（用户表示没看懂需重解释）：候选① stats 页「打卡日历」月视图(纯浏览历史,推荐)；候选② home 页日历弹层(与 G3 补卡共用,易混淆)。
...
46. ✅ **v2.1 设计稿 PNG-first 流程采纳 + 图片 base64 修复（2026-07-22）**：用户明确要求设计稿走「先出 PNG 确认效果 → 再出/定稿 HTML」流程（一步甩完整多帧 HTML 稿易错不易 review）。同时定位「较多缺失」= 初版用相对路径图片(违反 #46)在预览里裂图；已将 6 处图片全部 base64 内嵌，Chromium 渲染验证通过。🔜 后续设计稿一律：base64 内嵌图片 + 出稿前 Chromium 渲染 PNG 自查 + 用户确认 PNG 后再定稿 HTML。关联 #46/#64、§3.21。
