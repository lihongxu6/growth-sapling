...
| 57 | 2026-07-20 | v2.0 头像改用 `wx.chooseImage`（album+camera）选图，《隐私保护指引》原选「未采集用户隐私」，但提交审核时平台弹窗已实测检测到隐私接口 **Album**；App 内未弹隐私授权框 | 微信隐私规则（基础库 2.32.3+）：chooseImage/chooseMedia 等涉及相册/摄像头的能力，须后台《隐私保护指引》声明对应 scope 且用户授权弹窗通过后才能调用；否则 enforcement 设备直接失败、提审可能因「未声明却调用」被拒，或发布后权限被回收 | ① 用户已点击提交，平台提示 Album → 必须点「取消」，先在公众平台《隐私保护指引》声明相册(scope.album)+摄像头(scope.camera)，再重新提交审核；② App 内调 chooseImage 前补 `wx.requirePrivacyAuthorize` 授权弹窗（基础库 2.32.3+）；③ v1.0 攻略「未采集隐私」对 v2.0 不适用，指引须更新；④ 见 #40/#41、微信小程序提审攻略-2026.md | 本文件 §3.15、§6 待办 41、攻略 v2.0 注 |
