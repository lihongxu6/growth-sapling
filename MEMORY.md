...
- 框架草稿 `docs/需求框架-用户引导功能.md` 已迭代到 **v0.2**（含 G1–G4 + R1–R3 规则 + 3 项待确认设计分叉），待用户确认后写 PRD。关联 #57、§3.8（不堆独立页，用单页操作手册）、§3.10、§3.15。
...
42. 🔜 **下个版本 = 用户引导功能（2026-07-20 用户确认 + 具体范围已拍板）**：① 受众=儿童；② 首次引导=交互式操作引导（清空数据→走 建任务/打卡/补卡/看日历/看徽章/改头像昵称，所建任务沉淀真实账户不删）；③ 成长页新增常驻帮助页/操作手册（图标入口）。流程（用户确认）：**需求框架讨论确认 → 落到 `prd-成长小树苗.md`（EARS 新版本章节）**。**状态**：已进入**阶段一·需求规划**，框架 `docs/需求框架-用户引导功能.md` 已迭代到 **v0.2**（G1–G4 + R1–R3 + 3 项待确认分叉：帮助页形态/可跳过/强控）；决策沉淀见 §3.20。待用户确认 3 项分叉 → 写 PRD → 设计稿 → 技术方案 → 研发。启动条件仍记：2.0.1 审核结果 + 《隐私保护指引》声明审核通过。关联 #57、§3.8、§3.10、§3.15、§3.20。
