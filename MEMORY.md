...
33. 🔄 **H5 优化进行中**（2026-07-19）：H5 与小程序已分叉（§3.12）。当前已做本地改动（未 push）：① 顶部状态栏+导航栏移除；② 我的成长页用户信息卡片移除；③ 所有松鼠形象统一用浇水松鼠 IP（空状态/庆祝/徽章动画）；④ 日历弹层支持点击灰色背景关闭。等待用户确认后 commit + push GitHub Pages。
