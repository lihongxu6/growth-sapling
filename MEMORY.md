...
- 框架 `docs/需求框架-用户引导功能.md` 已迭代到 **v0.3（已确认）**：补 3 项分叉决定（帮助页=纯静态图文 / 首次引导可跳过 / 弱引导）+ **松鼠果果拟人化引导**（目的=拟人、建立情感连接；当前用占位形象，真实 IP 到位替换，见 §1）+ R4/R5 规则。PRD 已落地 `prd-成长小树苗.md`「**版本 2.1**」章节（EARS：G1–G4 + 核心流程 + 验收 + 边界 + 埋点建议）。关联 #57、§3.8、§3.10、§3.15。
