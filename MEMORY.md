...
### 3.31 引导气泡贴近目标 + 三角小尾巴（2026-07-22 用户拍板）
...
47. 🔄 **v2.1 设计稿第二轮重做（方法 B+混合方案+真实徽章，2026-07-22）**：用户对 6 帧稿给出 4 点反馈：① 徽章不匹配真实 BADGE_DEFS（5/6 编的，#66）→ 全部换成源码 6 个真实徽章；② G3 补卡太长（§3.30）→ 砍掉改 6 步；③ 气泡统一在底部（§3.31）→ 按目标位置动态锚定 + 三角小尾巴；④ 引导形态决策（混合/全真实/全概念，§3.29）→ 用户拍板**混合**：G1+G2 真实逐步带预填示例，G3–G6 概念性看图。**当前进度**：用户已确认方案 → 进入 HTML 重写 + Chromium 渲染 PNG 自查 + 提交确认（§3.2 两步确认）→ 确认后 commit。设计稿重写后切架构师角色写 tech-spec v2.1（页面/组件/状态/埋点）。关联 #66/#67、§3.25/§3.26/§3.27/§3.28/§3.29/§3.30/§3.31。
