...
**v2.0 状态**：我的成长页用户信息合并设计稿已修复并重新渲染（2026-07-18）。标题位置经用户确认保持 A 方案（规范居中，不改）。用户同时提出将 H5 开屏动画移植到小程序；`design-spec` §3.1.1/§4.1/§L11 已定义该开屏页，mini-program `app.js` 中留有 `splashDone` 字段但页面未实现。待用户确认是否纳入 v2.0 范围后进入研发。
...
27. 🔄 开屏动画移植需求（2026-07-18）：用户希望将 H5 版开屏动画（🌱 + 成长小树苗 + 陪你慢慢长大，1.5s 淡出，点击可跳过）移植到小程序。`design-spec-成长小树苗.md` §3.1.1/§4.1/§L11 已定义该开屏页；`miniprogram/app.js` 中留有 `splashDone` 字段但页面未实现，H5 版 `index.html` 已实现完整动效。待确认：纳入 v2.0 一起落地（推荐，可覆盖 v2.0 云登录/数据迁移初始化时间）还是单独作为后续优化项。
