...
- **老设备降级（概念澄清）**："降级"指基础库 < 2.21.2 时 `chooseAvatar`/`input type=nickname` 原生组件不可用，需 fallback（plain text input + 默认头像）。**≠ 本地存储**——wx.setStorageSync 是既定方案，不是降级对象。推荐方案 A：不额外做降级，要求基础库 ≥ 2.21.2（近全覆盖、最简单、契合 #3.14 避免 premature complexity）；待用户拍板。
