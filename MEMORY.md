...
4. 🔜 微信小程序适配（**进行中**）：打卡页 6 项问题已修复 + TabBar 图标方案 A 已应用，用户正在验证
   - 打卡页：日期栏/补卡banner/Tab图标/文案/火焰/进度条 ✅
   - 待处理：统计页和任务页的视频分析（用户已录制，待发送）
   - 编译链：miniprogram-ci preview ✅（用户关闭了 IP 白名单）
5. 可选：上线复盘报告、v1.1 功能规划
