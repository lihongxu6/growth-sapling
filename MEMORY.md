...
22. ✅ 用户信息合并方案已定（2026-07-18）：目标页=**成长页**（用户同意 Sophia 推荐）；头像获取=方案 A 已定（首次轻引导+就地点改）。**TabBar 调整已拍板**：顺序改「🏠打卡/📋任务/👤我的成长」，成长页置最右、标签更名「我的成长」（见 §3.9）。→ 下一步：① 重做合并版设计稿（成长页顶部用户信息区+底部退出登录+新 TabBar）评审；② 评审通过同步改 PRD/tech-spec v2.0 章节 + `design-spec` §3.12/§4.1；③ 云开发环境由用户在微信公众平台开通（并行）。原 `user-profile-design.html` 独立页稿已作废，重做为合并版。
