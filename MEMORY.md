...
- **核心理念**：**帮助孩子自我管理**，而非家长监督工具
  - **IP 形象**：小松鼠“果果”——**二期已完成设计落地**。真实松鼠 IP 资产已应用于小程序：`miniprogram/assets/avatar-144.png`（默认用户头像 `stats.js defaultAvatar` + 全部完成庆祝弹窗 `home.wxml` celeb-squirrel）、`miniprogram/assets/headers/`（icon-checkin / icon-growth / icon-task 页面头部图标，松鼠主题）。🐿️ emoji 占位已弃用；早期 ImageGen 生成失败属尝试性坑，已被二期正式设计资产取代。
...
- **用户卡提示文案**：用户信息区**不显示常驻提示**（如"点击修改头像/昵称"）。用户决策：头像/昵称修改是**极低频操作**，不应持续占用页面、增加认知噪音；仅靠头像右下角 ✎ 铅笔角标作为可编辑暗示即可。
- **我的成长页 page-header**：仅「我的成长」页移除 page-header（用户卡作页面主体）；打卡页/任务页的 page-header **保留**（用户确认，2026-07-19）。
...
- **隐私声明（概念澄清 = 两件事）**：① 微信公众平台《隐私保护指引》后台声明（告诉平台收集啥，仅当调用需声明接口如 getUserProfile/位置等才强制）；② 应用内透明提示文案（自己写，不依赖 API）。**反转（2026-07-19 用户明确）**：之前"采纳应用内透明提示"的决定**撤销**——用户指出小程序里用户留下的**所有数据都是隐私**（不止昵称/头像，还有打卡/任务等），把隐私提醒零散塞进某功能的技术方案阶段是错位的。→ **决策：隐私/透明提示不在当前技术方案阶段加**，推迟到**下一版「小程序使用引导规划」**，从**界面设计**整体做（与 #1 首次引导同属下一版统一引导）。本期本地版用户信息区**不含**任何隐私文案（应用内透明提示仍推迟到下一版引导规划）。注：v2.0 因 `wx.chooseImage` 用到相册/摄像头，平台**强制**要求《隐私保护指引》声明相册+摄像头（见 #57），这与「应用内不零散加隐私文案」不冲突——#41「未采集隐私」对 v2.0 不再适用。**2026-07-20 用户确认下个版本 = 用户引导功能**（首次轻引导 + 隐私透明提示），即本节所说的「下一版小程序使用引导规划」，从界面设计整体做。
...
- 与 tech-spec §2.0.7（云版）关系：本地版是其推迟期派生实现；用户确认 3 项后写入 tech-spec §2.0.6（本地版）。
- **默认昵称与头像显示（2026-07-19 用户真机反馈）**：用户真机截图显示当前默认昵称为 "设置昵称" 且头像显示不全；用户明确要求按原型改为 **"果果的好朋友"** 作为默认昵称。头像显示不全根因：`<image>` 使用 `mode="aspectFit"` 导致内容未填满圆形头像区；修复方案：改用 `mode="aspectFill"` 使松鼠头像完整填充圆形，同时让 ✎ 编辑角标正确落在头像上。待确认：默认昵称是（A）作为预填充值并自动持久化到 storage，还是（B）仅预显示但首次不写入 storage、等用户编辑时再存。**已决策 A 并落地（2026-07-19 用户确认 via q-0）**：首次无昵称时预填「果果的好朋友」并自动持久化到 storage（profile.nickname='果果的好朋友'）。头像修复 `mode="aspectFit"` → `aspectFill` 已改 `stats.wxml`；默认昵称逻辑已改 `stats.js` `_loadProfile`；重新 compile ✅ + 重新出 preview 二维码 ✅；代码提交 `f5b0ad4`。自测脚本 `scripts/test_userinfo.js` T-U04 已同步为「首次默认果果的好朋友并持久化」，复跑 13/13 ✅。详见 `docs/测试验收报告-用户信息区v2本地版.md` 第七节。**二次迭代（2026-07-19 真机复核）**：仅改 `mode="aspectFill"` 仍不满圆——根因是圆形裁剪与 ✎ 角标定位**共用 `.avatar-wrap`**、且 **button 默认尺寸把容器撑大**；只改 mode 不够。已重构三层：`.avatar-circle`（定 112×112、`border-radius:50%`、`overflow:hidden` 真裁剪 + 绿边框）→ `.avatar-btn`/`.avatar`（100% 填满）→ `.avatar-badge` 移回 `.avatar-wrap`（不被裁、绝对定位右下）。结果：松鼠头像完整填满圆形、✎ 角标正确落在头像右下。代码提交 `975ef83`，preview 重出。**第三轮（2026-07-20 真机复核）**：用户反馈①默认头像没了（圆圈只剩绿底）；②选照片没居中。根因①：此前扫旧二维码选过照片，`profile.avatarUrl` 存的是上一次预览会话的 `wxfile://` 临时路径，换二维码后失效，但 `src="{{avatarUrl || defaultAvatar}}"` 中 `avatarUrl` 仍为真值覆盖了默认松鼠（跨预览会话缓存回收的 stale 路径，非资源丢失）。修复：`_loadProfile` 改为「先按默认头像渲染（`avatarUrl=''`），再 `wx.getFileSystemManager().access` 异步校验已存头像是否有效，通过才回填、失败则回落默认并清理存储」；低版本无 `access` 自动兜底。根因②：`<image>` 嵌在 `<button open-type="chooseAvatar">` 内受按钮内容流布局干扰，无法直接控制填满/居中。修复：图片与按钮解耦——`.avatar-circle` 内只放 `<image>`（直接子元素，`width/height:100%` + `mode="aspectFill"` 居中裁剪），`button` 改为浮在上面的透明触发层（`position:absolute; inset:0; opacity:0; z-index:2`），`.avatar-badge` 保持 `z-index:3; pointer-events:none`。自测 `test_userinfo.js` 新增 T-U13（stale 路径回落默认），**13/13 ✅**；compile ✅；preview 重出 `.preview-qr.png`（46704 bytes）。**第四轮（2026-07-20 真机复核）**：第三轮修头像后**回归**——点昵称却拉起换头像浮层（"拉起换头像的区域太大，在最右侧才能唤起改名字"）。根因：绝对定位透明覆盖层 `.avatar-btn` 用了 `width:100%;height:100%`，真机上 `%` 宽度**解析到视口 750rpx** 而非 112rpx 头像容器，覆盖层铺满整张卡片盖住右侧昵称（修头像时**未做昵称回归**导致）。修复：覆盖层改显式 `width:112rpx;height:112rpx`（相对 `position:relative` 的 `.avatar-wrap` 定位），严格限制在头像列内不再外溢；`.avatar-wrap` 维持 `position:relative`+`width:112rpx` 作包含块。回归测试（用户明确要求：改完一个功能点后做相关功能回归）：`scripts/test_userinfo.js` 新增 `T-W01`（断言按钮嵌在 avatar-wrap 内且在昵称之前、昵称 input 保留 bindblur/bindconfirm）+ `T-W02`（断言 `.avatar-btn` 必须显式 112rpx、禁止 width:100%），自测 **15/15 ✅**；compile ✅；preview 重出（47149 bytes）。详见验收报告第九节 + #52。新规则（用户明确）：**改完一个功能点后必须做相关功能回归**（见 §3.18）。**第五轮（2026-07-20 真机复核）**：第四轮显式 `width:112rpx` **仍未生效**——绝对定位覆盖层的命中区/包含块解析在微信真机就是脆点，纯技术精准控制命中区难。用户拍板：「技术不好实现就改产品方案」，在名字栏右侧加可修改图标引导点右边改名字。修复（去覆盖层 + 产品兜底）：① **去掉绝对定位覆盖层**——`.avatar-btn` 改为**头像即 chooseAvatar 按钮本身**（普通 flex 子元素，`border-radius:50%+overflow:hidden`，image 嵌内 `display:flex` 居中 + `mode=aspectFill` 填满裁剪）；它是 `.user-card` 的普通 flex 子元素，被布局死死限制在 112rpx 头像列内，**物理上不可能外溢到昵称**（彻底绕开视口-% 解析脆点）；② **昵称右侧 ✎ 编辑图标**（用户提议的产品方案兜底）：`.user-meta` 改 flex 行，昵称 `<input>` + 右侧 `.name-edit`（`bindtap=onEditNameTap` 聚焦输入框），给明确独立的「改名字」点击目标，引导点右边。回归（延续 §3.18）：`T-W01~T-W04` 扩到新结构（含**禁止 `position:absolute` 覆盖层** T-W03、昵称右侧必须有可点击 `name-edit` T-W04），自测 **17/17 ✅**（13 逻辑 + 4 静态结构）；compile ✅；preview 重出（47274 bytes）。详见验收报告第十节 + #53。该方案已回滚（2026-07-20 用户反馈布局严重回归：头像变椭圆、昵称覆盖头像），代码恢复至第四轮 `2b598d8` 状态。

**第六轮（2026-07-20 用户拍板 C 方案）**：回滚后第四轮 112rpx 覆盖层仍真机失效，命中区冲突未解。用户从交互设计师给出的 A/B/C 中选 **C**：`<view>` 头像 + `catchtap→wx.chooseImage` 自裁剪（弃用原生 chooseAvatar 浮层），**再加**昵称右侧独立 ✎ 编辑图标双保险。落地：① 去 `<button open-type="chooseAvatar">` 与 `.avatar-btn` 绝对定位覆盖层；② `.avatar-circle` 改为 `catchtap="onPickAvatar"`（仍是 .avatar-wrap 内 112rpx 盒子，无绝对定位、无 button）；③ JS 的 `onChooseAvatar` 改为 `onPickAvatar`（调 `wx.chooseImage` → `saveFile` 持久化，stale 回落 #51 逻辑保留）；④ 昵称 `<input>` 右侧加 `.name-edit`（`catchtap="onEditNameTap"` → `wx.createSelectorQuery` 聚焦输入框）；⑤ 自测扩到 **16/16 ✅**（T-U06/07/12 改驱 onPickAvatar + 新增 T-U14 onEditNameTap 不抛错 + T-W01/T-W02 重写：弃用原生 button、命中区死卡 112rpx、昵称右侧有 name-edit）；compile ✅；preview 重出。详见验收报告第十二节 + #55。待用户真机扫码确认（§3.18 回归：点头像只换头像、点昵称/✎ 只改名字、两者互不误触）。
...
**第九轮（2026-07-20 真机复核 · 未来日期不着色）**：用户扫码第八轮二维码后指出，未来日期（21、22、23 等）仍显示颜色，但**未来日期不应该着色**——任务还没到可打卡时间，保持白底才对。用户明确拍板：未来日期无论是否有任务、是否有完成记录，都保持白色（白底），只有当天及历史日期按方案 B 着色。修复：`_buildCalGrid` 中先把 `isFuture = date > todayDate` 计算出来，着色逻辑包在 `if (!isFuture)` 内；`backfill` 也改为 `let` 以便未来日重置为 `false`；未来日仍保持 `disabled` 不可点击。新增 T-C06 动态断言（未来日白底）+ T-W04 静态断言（含 `if (!isFuture)` guard）。沙箱自测 23/23 ✅，preview 重出。与第八轮同属日历配色修复，尚未上传（仍需重新 upload 2.0.1）。

**第十轮（2026-07-20 真机复核 · 日历口径与打卡页对齐）**：用户真机发现 7 月 13 日在日历上显示**绿色**（全完成），但点进去发现还有 2 项任务未完成。根因：日历 `_buildCalGrid` 用的是**全局未删除任务总数**作为分母，且完成数直接数 `checkinsByDate[iso]` 里的 done 记录，没有按当天重复规则过滤；而打卡页 `getActiveTasks()` 按 `repeat_type` 算出当天活跃任务。口径不一致导致日历误判。修复：日历逐日按 `dayOfWeek` 重新算 `activeTasks`（与 `getActiveTasks` 同逻辑），完成数只统计「当天活跃任务中 `ci[t.id].done=true`」的数量；分母/分子都和打卡页一致。新增 T-C07（5 活跃完成 3 → 橙）+ T-C08（工作日过滤周末任务）。沙箱自测 25/25 ✅，preview 重出。见 #60 + 验收报告第十七节。
...
- **设计师角色实例（2026-07-19）**：用户切 Sophia 回设计师角色，清理 `ui-mockup-v2-userinfo.html` 中残留的「退出登录」（标题/CSS/HTML/图例），并同步 `design-spec`（§3.12/§4.1）、`PRD`（§2.0.3 功能清单）、`tech-spec`（§2.0.10 注）对「退出登录」的描述，使所有交付物一致指向「当前本地版无退出登录，退出登录随账户体系（云版）推迟」。；并进一步删除原型中残留的「修改资料」底部浮层弹窗（与已定「头像直接选 / 昵称失焦即改」技术方案冲突），图例同步改写为「无修改资料弹窗」，用户卡去掉右下 chevron 箭头（已无整卡下钻语义）。
- 后续用户反馈用户信息区提示文案不应常驻（头像/昵称修改极低频），Sophia 先切**用户体验师/交互设计师**视角给出判断：常驻提示会造成认知噪音，建议删除、仅保留头像右下角 ✎ 铅笔角标作为可编辑暗示；用户确认后删除提示文案并同步图例。
...
- **交付物与标准**：
  - 架构师（阶段三）：交付 = 研发设计方案（EARS 需求 + 任务拆分 T1–T10 + 微信官方约束核验 #17/#QC7 + 验收标准）。标准 = 用户确认定稿 + 写入 tech-spec 并 commit+push 到资料库。
  ...
  - **测试前置（用户 2026-07-19 明确）**：在把真机二维码交给用户扫码前，必须先以**测试工程师**身份做一轮沙箱自测，排除可测工程问题——不可把"扫码验证"直接甩给用户。沙箱可测项：静态/编译核查(QC3/4)、逻辑单测(Node + mock wx：getProfile/setProfile 往返、昵称 trim/空值跳过、saveFile 成功持久化/失败保留原图、onLoad 读取)、回归(QC5/6 + §5 L-list 逐项)。**硬约束**：chooseAvatar / input type=nickname 是微信原生组件，DevTools/沙箱均无法模拟真机行为（#22/#34/#QC7），该部分交互验证**只能**留待用户真机扫码确认，这是平台限制不是偷懒——验收报告须逐项标"沙箱已验 / 待真机确认"。
...
- **透明提示文案候选（已作废）**：原 A/B/C 因用户反转决策（隐私推迟到下一版引导规划）**作废**，本期不加。保留备下一版引导规划参考：A「📱 数据只存在这台手机，不会上传」/ B「打卡记录仅存本机 · 不会上传网络」/ C「你的成长数据都在本机，放心用～」。

---

### 3.18 改完一个功能点必须做相关功能回归（2026-07-20 用户明确）

- 用户原话：「记住改完一个功能点后要做相关功能的回归测试」。含义：修复/重构某一个功能点（如头像）后，**不能只验证该功能本身**，必须主动对**强相关的其他功能**（如同一卡片内的昵称编辑、兄弟节点的交互）做一轮回归——逻辑自测 + 真机复核都要覆盖，避免「修 A 坏 B」的回归漏测。
- 硬约束：chooseAvatar / input type=nickname 是微信原生组件，沙箱测不了，相关交互回归**必须留待用户真机扫码确认**（#22/#34/#QC7），这是平台限制不是偷懒；但能在沙箱静态/逻辑层防住的结构性回归（如覆盖层是否外溢盖住兄弟节点）要用断言卡死（见 #52 的 T-W01/T-W02）。
- 纪律衔接：与 §3.17 测试前置一致——交付真机二维码前先跑沙箱自测（含回归断言），再交用户真机复核。
- 补充（2026-07-20 第五轮）：当某交互的**精准技术实现在真机脆/难控**（如微信绝对定位命中区解析），用户已拍板可接受**改产品方案兜底**（本例：昵称右侧加独立编辑图标引导点右边），不必死磕脆弱技术。但**回归纪律不变**——改完仍要做相关功能回归（T-W01~T-W04）。

---
### 3.19 补卡完成庆祝弹窗（2026-07-20 用户拍板）
- 用户建议：补卡完成时也应给弹窗庆祝（与今日完成一致的正向反馈，鼓励坚持）。
- 决策：① 触发条件放开——不再仅限 `isToday(viewDate)`，只要「当天最后一个活跃任务完成」就弹一次（今天 / 补卡日统一口径）；② 文案动态化：`home.js` 设 `celebrationText` 数据字段，今天显示「今天的任务全部完成！」，补卡日显示「X月X日的任务全部补卡完成！」（`fmtDateShort(viewDate)` 生成日期）；③ 动画与松鼠图标**复用现有 celebration-overlay**，仅改文案、视觉保持一致；④ 防弹窗轰炸——只在「最后一个活跃任务完成」时弹，不在每次打卡都弹。
- 落地：`pages/home/home.js`（`data.celebrationText` + 放开 `doneAfter >= totalActive` 触发）、`pages/home/home.wxml`（绑定 `{{celebrationText}}` 替代写死文案）；新增 `scripts/test_home.js`（T-H01 今天/T-H02 补卡/T-H03 未完成不弹/T-H04 文案动态）4/4 ✅。详见验收报告第十八节。

### 3.20 用户引导功能决策（2026-07-20 用户确认）
- 下个版本 = 用户引导功能，具体范围用户已拍板：① **受众=儿童**（文案童趣/图文，家长可协助）；② **首次引导=交互式操作引导**：清空数据→引导儿童走 创建任务→打卡→补卡→看日历→看徽章→改头像昵称，尽量覆盖核心功能点，所建任务沉淀为真实账户数据（**不删**，引导即首次真实使用）；③ **成长页（我的成长）新增常驻帮助页/操作手册**：设计一个图标，点击查看操作手册（图文步骤说明）。
- 框架 `docs/需求框架-用户引导功能.md` 已迭代到 **v0.3（已确认）**：补 3 项分叉决定（帮助页=纯静态图文 / 首次引导可跳过 / 弱引导）+ **松鼠果果拟人化引导**（目的=拟人、建立情感连接；真实 IP 已落地，见 §1，引导与手册直接用真实资产）+ R4/R5 规则。PRD 已落地 `prd-成长小树苗.md`「**版本 2.1**」章节（EARS：G1–G4 + 核心流程 + 验收 + 边界 + 埋点建议）。关联 #57、§3.8、§3.10、§3.15。

### 3.21 设计稿交付流程偏好（2026-07-22 用户明确）
- 用户期望的设计稿流程 = **先出 PNG（视觉图）确认效果 → 再出/定稿 HTML 原型**，而不是一步甩出完整多帧 HTML 稿（易错、不易 review，用户原话“一步出 HTML 感觉有好多错误”）。
...
- ⚠️ **2026-07-22 末次修正（覆盖本节约「先出 PNG 确认」部分）**：v2.1 设计稿改为 **HTML 单文件交付（单一真相源）**，详见 §3.35。PNG 不再作为需维护的交付物——理由：① #69 Chromium headless emoji 渲染 bug 曾致 3 轮 PNG 全空白、误导评审；② 维护 PNG+HTML 双文件有漂移/不一致风险；③ 用户可直接在浏览器打开 HTML 校对，最准。PNG 降为「按需从 HTML 经 Chromium 渲染的一次性预览」，不 commit、不手改，故无源漂移。
...
### 3.26 v2.1 引导流程定稿（对齐中，2026-07-22 用户修正）
- **删除 S1「欢迎+清空」**：用户确认用户侧引导第一步直接是「创建任务」，无需欢迎仪式页。
- **「清空」性质澄清（重要）**：PRD 写的「清空数据」是**后台/开发过程的数据清理**（开发期带上去的、不属于用户的测试数据），用户根本无感知，**不应出现在引导流程里，也不该告知用户**。即 PRD「清空数据→建任务」中的清空是 dev-side 准备动作，非 user-facing 引导步骤。
- 修订后 user-facing 流程起点 = **创建任务**（其后待确认：打卡→补卡→看日历→看徽章→改头像/昵称→完成）。
- 旧草稿 `ui-mockup-v2.1-guide.html` + `design-draft-v2.1-guide.png` 已作废（流程错序 + 凭想象背景），待流程锁定后以真实页面背景重做。

### 3.27 v2.1 引导真实页面/功能映射（2026-07-22 读源码核实）
- 真实页面仅 4 个（app.json pages）：`splash`(启动)/`home`(打卡 tab)/`tasks`(任务 tab)/`stats`(我的成长 tab)。**无独立「创建任务页/日历页/徽章页」**——此前设计稿虚构这些页已作废(#65)。
- 真实功能落点：① 创建任务=`tasks`页「添加新任务」(onAdd)→弹层(图标/名称/意义/重复周期)→「添加」(submitForm)；② 打卡=`home`页任务卡(onTaskTap)→✅→庆祝弹层；③ 补卡=`home`页日期(openCalendar)→日历弹层选过去日(pickDate)→补卡模式打勾；④ 看日历/看徽章/改头像昵称=`stats`页 section（打卡日历月视图 / 🏅徽章墙 / 头像onPickAvatar+昵称input）；⑤ `home`页空状态写明「去『任务』页添加」→证实打卡页无加任务按钮，必须先建任务再打卡。
- `stats`页**当前无「常驻手册入口」**：手册入口(G4)是 v2.1 新增功能，需技术方案新增，设计稿里它是新元素而非现有功能。
- 引导正确形态=**真实页面之上叠加上下文引导浮层**，每步标注[真实页面|真实功能]并高亮该页真实按钮/区域；不新建页面、不虚构功能。

### 3.28 v2.1 引导流程锁定项（2026-07-22 用户拍板）
### 3.29 v2.1 引导方案 = 混合（2026-07-22 用户拍板）

- **核心 2 步真实逐步引导** + **探索性 4 步概念性看图**，平衡「真做才会做」与「流程不疲劳」。
- **真实逐步**：G1 添加任务（带预填示例）、G2 今日打卡（点任务卡打勾）。孩子不打字，只点保存/打勾 = 完成真动作。
- **概念性**：G3 看日历、G4 看徽章、G5 改头像、G6 手动补记。仅看图+气泡说明，不强制操作。
- 业内依据（Userpilot）："Clicking 'Next' confirms only that the user saw the instruction. Clicking the product element starts building the behavior you want the user to repeat after onboarding is over."（driven-action 优于 passive coachmark for new-user activation）。
- 7-8 岁孩子耐心短，6 步总时长控制在 60 秒内为佳。

### 3.30 v2.1 引导砍 G3 补卡，6 步定稿（2026-07-22 用户拍板）

- **砍 G3 补卡**（home 日历选过去日）：用户判断「补卡属于逆向操作、加进来流程太长」，且会暗示「漏打卡是常态」与产品正向激励理念冲突。
- **最终 6 步**：G1 添加任务 / G2 今日打卡 / G3 看日历(stats 月历) / G4 看徽章 / G5 改头像 / G6 手动补记(手册入口)。
- 原 7 步 → 现 6 步，每步仍锚定真实页面+真实功能（§3.27 映射不变）。

### 3.31 引导气泡贴近目标 + 三角小尾巴（2026-07-22 用户拍板·被§3.32 替换为「松鼠说话」方案）
### 3.32 松鼠说话设计（角色化引导，2026-07-22 用户拍板）

- 用户对 v2.1 第二轮 6 帧设计稿反馈：气泡贴近目标的定位不准确（设计稿里无法做到像素级精准）。
- **用户方案**：把松鼠从气泡里拿出来，作为独立的「角色」站在页面右下角。气泡来自松鼠，作为「果果对你说的话」，不需要指向具体目标——目标靠 `.target` 高亮识别，气泡只承担「这句提示是果果说的」角色。
- **落地**：① `.guide-squirrel` 固定右下角（100×100px），用 `transform:scaleX(-1)` 翻转使其「看屏幕外、面向用户」；② `.guide-bubble` 在松鼠左侧（max-width 240px），气泡尾巴朝右指向松鼠；③ 6 帧统一一套松鼠+气泡布局，不因目标位置变化而动。
- **效果**：解决像素级定位不准确的硬问题，同时把 IP 拟人化做到位（果果是引导者，不是 UI 装饰）。

### 3.33 G1 添加任务弹窗按线上真实结构复刻（2026-07-22 用户拍板）
### 3.34 Chromium headless 必须显式声明 emoji 字体才能渲染（2026-07-22 踩坑）

### 3.35 v2.1+ 设计稿交付 = HTML 单文件（单一真相源；覆盖 §3.21「先 PNG 确认」部分，2026-07-22 用户拍板）
- 决策：**设计稿只出 HTML 单文件**，不再维护独立的 PNG 交付物。用户原话：「HTML 要是必经之路的话，我就觉得应该直接出 HTML」「维护这两个文件会不会引入不一致」。
- 落地：① HTML 图片一律 base64 内嵌（守 #46），自包含、无裂图；② emoji 字体显式声明（§3.34）；③ 出稿前 Chromium 渲染 PNG 仅作「自查/一次性预览」，不 commit、不手改——PNG 永远是 HTML 的派生物，单一可编辑源 = HTML，故不存在双文件漂移；④ 用户评审直接浏览器打开 HTML（最准，避 #69 emoji 坑）。
- 与 §3.21 关系：§3.21「先出 PNG 确认 → 再定稿 HTML」流程作废（被本约覆盖）；其余（base64 内嵌、Chromium 自查）保留并并入本约。
- 注：本约不影响「AI 出图不依赖生成式工具」原则（§3.21 末段仍成立）。

- **坑**：沙箱 `apt install fonts-noto-color-emoji` 不可用；手动下载 NotoColorEmoji.ttf + `fc-cache` 后，fontconfig 能 `fc-match emoji` 命中 Noto Color Emoji，但 Chromium headless 仍渲染 emoji 为空白格。
- **根因**：Chromium 字体回退策略不主动使用 Color emoji 字体，必须 CSS 中显式声明。
- **解法**：在 HTML `<body>` 的 `font-family` 栈追加 `"Apple Color Emoji", "Noto Color Emoji", "Segoe UI Emoji"`。声明后 emoji 正常渲染（饱和像素检测可验证）。
- **PNG 自检扩展**：除尺寸/像素裁剪外，加一道「饱和像素检测」——`max(R,G,B)-min(R,G,B)>50` 的像素占比 > 0.05%，证明有彩色内容（emoji/插画），否则说明 emoji 没渲染。
- **设计稿图片 base64 内嵌**（#46）+ emoji 字体显式声明：双管齐下，PNG 与 HTML 一致。


- 用户反馈：v2.1 第二轮 G1 弹窗凭印象画，多处与线上 `pages/tasks/tasks.wxml` 不符。
- **真实结构（必须对齐）**：① 标题「添加任务」（非「添加新任务」）+ X 关闭按钮（form-close）② 图标选择 = `iconSet` 16 项（不是凭感觉凑）③ 任务名称/意义 是 input（带 placeholder）④ 重复周期 4 选 1：每天 / 工作日 / 周末 / 自定义⑤ 自定义显示 week-row（周一~周日）⑥ 底部 取消 + 添加 两个按钮并排（不是单个「添加」按钮）。
- **预填规则**：布局 100% 真实，但表单内容允许果果预填示例（孩子不用打字）。
- **判分原则**：设计稿渲染前必须 grep 源码 `tasks.wxml/wxss` 对照结构，错 1 处都算失职。


- 用户反馈所有气泡统一在页面底部，远离高亮目标。
- **新规则**：气泡位置按目标位置动态锚定 + 三角小尾巴指向高亮元素：
  - 目标在屏幕中下部 → 气泡放在目标**上方**，尾巴向下指
  - 目标在屏幕顶部 → 气泡放在目标**下方**，尾巴向上指
  - 目标在屏幕外（需滚动）→ 气泡先于滚动出现，文案「向下滑动看更多」
- 适用所有 v2.1 设计稿与未来引导类视觉稿（设计稿 HTML/CSS 阶段就要按此实现，不得全部固定底部）。

- G6 改头像：**引导中不真触发** G3 `wx.requirePrivacyAuthorize` 隐私弹窗——只引导到「点头像」之前并提示"点这里会触发相册授权弹窗"，避免引导副作用。
...
### 3.37 v2.1 日历三色语义对齐上线（2026-07-23 用户确认）
- **上线真值**：统计页月历 = ① 🟢 绿色 = 全完成（所有任务 done）；② 🟠 橙色 = 还没打卡（该日有任务但 0 完成；**上线不区分**「0 完成」与「部分完成」，均=橙色「还没打卡」）；③ ⚪ 灰色 = 没任务（该日无任务，不需打卡）。
- **设计稿落地**：G3（我的成长页·打卡日历）气泡图例已改色块 emoji `🟢全完成 🟠还没打卡 ⚪没任务`；日历无任务日格已批量加 `gray` 类（120 处）。详见 `ui-mockup-v2.1-guide.html`。
- **纪律**：设计稿任何颜色/数据/图标/文案必须**以上线小程序现状**为准（产品 owner 是唯一真相源），未落地的修复方案（如原 #58 提出的「黄色=0完成 / 橙色=部分完成」）**不能**用作设计稿；先确认上线真值再设计。
### 3.38 v2.1 操作手册设计落地（G6 入口 + G7 弹层，2026-07-23 已落地）

**用户原话回顾**："这两个问题都按你的建议来。你设计一下，我看一下效果。" —— 两个建议即 §3.38 初稿（内容复用 6 步 / 入口做小）。

**G6 入口（已落地）**：`.manual-entry` 由"绿底大块 + 📖 + 标题 + 副标题 + `tag-new` 新增角标"**改为成长页底部单行小文字链接**：
- 呈现：`📖 操作手册`，灰小字、居中、无背景、无边框、无角标。
- 选中态：`:active` 文字变绿（按压反馈）。
- 仍保留 `data-target=".manual-entry"` 让 G6 引导聚焦它，避免分散孩子注意力，也避免在 TabBar 新增项（守 §3.9）。

**G7 操作手册弹层（已落地）**：点 G6 的链接后从底部弹出的 bottom sheet：
- 形态：`sheet-mask`（暗化背景） + `manual-sheet`（白底圆顶卡片，从底部上滑）。
- 内容：把 6 步引导**脱壳** = 去掉遮罩/聚焦环/跳过/下一步，只留 5 张干净卡片：
  1. 📋 添加任务 — 写下想养成的小习惯，从一个小目标开始
  2. ✅ 今日打卡 — 每天完成小习惯，点圆圈打勾攒星星
  3. 📅 看日历 — 点月历看进度：🟢全完成 🟠还没打卡 ⚪没任务（顺带强化 §3.37 三色语义）
  4. 🏅 看徽章 — 收集星星，解锁你的成长徽章墙
  5. 🖼️ 改头像 — 换个喜欢的果果头像，装扮小树苗（用 🖼️ 而非 🐿️，守"全稿仅 1 只松鼠"）
- 底部"再走一遍引导 ›"绿色按钮：点击可重播首次引导。
- 文件：`ui-mockup-v2.1-guide.html` G7 帧（`scripts/_make_g7.py` 程序化复制 G6 + 配平脱壳 + 注入 sheet，避免手写 base64 风险）。

**G6 引导文案同步**：聚焦环 + 气泡仍由 `data-target=".manual-entry"` 触发，文案改为：
> "以后不会用的时候，点这里看操作手册，果果带你一步步来📖 随时都能回看哦！"

**自检通过**：Playwright `verify_guide.py` 7 帧全 OK（G1–G6 定位校验 OK；G7 无 coach 跳过校验）。`bubbleGap 0–12px`（小尾巴桥接），`ringErr 0px`，无溢出。

### 3.39 自检指标修正：bubble 距离从"中心距"改为"边缘间隙"（2026-07-23）
- **动机**：G4 高亮徽章墙（目标 h=466）时，bubble 只能放顶上，中心距 ≈ 294 px，已超阈值 ≤ 260。但视觉上 bubble 底部紧贴目标顶端（gap ≈ 12 px），被小尾巴桥接 —— **"中心距"对高瘦/扁平目标有惩罚**，是错指标。
- **修正**：`verify_guide.py` 的 `dist` 由 `sqrt((bcx-tcx)^2 + (bcy-tcy)^2)` 改为两矩形 **最近边间隙**（相交即 0），阈值 ≤ 40 px。
- **结果**：G1–G6 `bubbleGap = 0 ~ 12 px`，全部 PASS；G7 无 coach 跳过。真正关心的是"气泡是否贴住目标且由尾巴桥接"，不是几何中心是否接近。
- **戒条（加入 §4 教训）**：自检指标必须**直接对应视觉诉求**，不要把"易于写成一行公式"的指标当真实问题（中心距看似严谨，实则在常见场景给出误导性 FAIL）。



### 3.40 "刚完成首次打卡"状态数据自洽（2026-07-23，用户反馈 + 同步推论）
- **触发**：用户指出 G7 弹层背后的"累计星星 = 8 颗"和"引导刚完成打卡"叙事对不上，应是 1 颗。
- **修正**：G3–G7 五帧统一：
  - `累计星星` 8 → **1**
...
- **3 个待确认（≤3）→ 已于 2026-07-23 全部拍板，详见 §3.42**：① 默认 4 任务=保留；② 遮罩透明度=.45（否 .30）；③ 概念步=混合推进（G1/G2 必须真点，G3–G6 逐屏讲解看一眼点下一步，非跳过）。

### 3.42 v2.1 引导三项待确认 → 全部拍板（2026-07-23 用户逐一确认）
- **决策① 默认 4 任务 + 星星 +1（已确认）**：保留 `constants.DEFAULT_TASKS` 真实预置的 4 个每日任务，不动产品播种行为（风险最低）。
  - 4 个真实默认任务：① 📖每日阅读30分钟（认识更多字）② ✏️认真完成作业（巩固今天学的）③ 🏃运动20分钟（身体棒棒的）④ 🧹整理自己的房间（学会照顾自己）。
  - 星星规则：`calcStars()` 统计"已打卡记录数"，每完成一次打卡 +1 颗★；引导叙事"刚完成首次打卡"→ **累计星星 = 1**（非 8、非 0）。
  - G2 演示态：列出 4 个默认任务、全部 `○` 未打卡，进度 `0/4`，高亮首个 `.task-card` 引导"点这里打卡"。
  - **连锁修正（本周完成率）**：`stats._calcWeekRate` 分母=7×4=28，首次打卡后完成率=**4%**（非 100%/25%）。设计稿本周环 `conic-gradient(绿 0% 4%, #EFE7DC 4% 100%)`、环心 `4%`、文案"今天完成 1 个小习惯，明天继续呀！"。
- **决策② 遮罩透明度 = .45（已确认）**：用户原话"要不 45 吧？30 的话是不是有点太淡？"。否定 §2 的 .30 提案——.30 过淡、非高亮区对比不足，不利 7–8 岁孩子聚焦。设计稿+真机均取 `rgba(0,0,0,.45)`。
- **决策③ 概念步混合推进（已确认，用户纠正"可一路狂点跳过"的误解）**：严格按 §3.28–§3.30 原方案。
  - G1/G2 = **强制真实操作步**：必须真点（G1 真加任务、G2 真打卡）才能解锁"下一步"，不可跳过（呼应"前两步必须点"）。
  - G3–G6 = **概念步**：每屏看图+一句讲解，孩子"看一眼点下一步"即可；不强求点目标元素，但**每屏独立讲解、须逐屏过**，不是"一路狂点下一步直接跳过"。
  - 与 §3.30"概念性看图不强求操作"一致，但保留逐屏认知节奏。
  - 设计稿已落地：G6 入口改为小底部链接、G7 新增脱壳 5 卡弹层（见 §3.38）。

### 3.43 v2.1 「跳过」按钮降权 / 移除·已落地（2026-07-23 用户补充决策）
- **触发**：用户认为「跳过 ✕」视觉太显眼（22px 圆角胶囊 + 文字 + ×），且**语义模糊**——点跳过是「结束整个引导」还是「等同于下一步」？
- **决策（4 项）**：
  1. **G1/G2 强制操作步 → 移除「跳过」按钮**（必须真点目标元素，不可跳）。
  2. **G6 末步 → 移除「跳过」按钮**（最后一步无可跳，只留「完成 🎉」）。
  3. **G3/G4/G5 概念步 → 「跳过 ✕」降权为小 × 图标**（22×22 透明灰圆，悬停加深，无「跳过」文字）。
  4. **小 × 语义 = 关闭整个引导**（`status=done`），**不等价于「下一步」**。
- **用户原话**：
  - "如果第一步和第二步是必须点的话，那么这里就不应该有这个跳过的这个图标"（G1/G2）
  - "3~6 步这个跳过...它跳过是跳到哪了呢？点跳过是把整个引导动画就终止了"（明确终止语义）
  - "我觉得这个跳过在视觉上太显眼了...放一个小叉就可以了，一个小叉用户的一嗯感知应该就是关掉"（视觉降权 + 关闭语义）
  - "最后一步上这个完成这一块，就不要有跳过了，因为都是最后一步了，也没啥可跳的了"（G6 移除）
- **设计稿落地**（`ui-mockup-v2.1-guide.html`）：
  - G1/G2/G6 头部仅留 `果果 · 第 N / 6 步`，无按钮
  - G3/G4/G5 头部右上角小 ×（22×22、`data-action="close-guide"`、`title="关闭引导"`）
  - 图例新增条目：「跳过」降权为小 ×（G3–G5）+ G1/G2/G6 无跳过
- **研发对接**（`tech-spec-v2.1-guide.md` §6.2 / §7 / §9 / §11.4 已同步）：
  - coach-mark 按 `step` 条件渲染关闭按钮：`step ∈ {3,4,5}` 才挂 `.bubble-close`
  - 事件名：`close` → `guide.close()`；`next` → `guide.advance()`；`complete` → `guide.complete()`
  - 埋点：`guide_close(step)`（替换原 `guide_skip`），`guide_complete(step,total_duration_ms)` 增加 `step` 字段
  - 样式：≤ 22×22px、半透明灰、悬停加深，绝不抢下一步/完成的主按钮视觉
- **新坑（待沉淀）**：研发条件渲染时务必确保 G6 「完成」按钮独占（不要被关闭按钮挤偏）；小 × 需 `catchtap` 避免穿透到目标元素。

### 3.44 verify_guide.py 的 PNG 截图 ≠ HTML 真值（2026-07-23 用户反馈，澄清认知）
- **触发**：用户在 HTML mockup 看到 G3 气泡头 `果果 · 第 3 / 6 步` 单行排版美观，但在 `_guide_frame_3.png`（verify_guide.py 用 Chromium headless 出的截图）里看到同一行被换行成 `果果 · 第 3 /\n6 步`，疑心是否需调整设计。
- **澄清**：
  1. **HTML 是设计唯一真相源**（§3.35 已明确），PNG 只是自检脚本顺手出的调试产物，**不进交付物**。
  2. PNG 里的换行是 **Chromium headless 默认字体宽度伪影**：
     - Chromium headless 跑 emoji/中文时用默认回退字体，比设计字体（PingFang/系统）**字符更宽**
     - 同一 `max-width:252px` 的 bubble，在 HTML 原生渲染能容下"果果 · 第 3 / 6 步"，在 Chromium 截图里就被挤到下一行
     - PNG 是 2× 缩放（788×1706），子像素抗锯齿也会让字看起来更胖
...
| 50 | 2026-07-19 | 技术方案已定「头像直接 chooseAvatar + 昵称失焦即改（无保存按钮、不跳页）」，但原型仍残留「修改我的资料」底部浮层弹窗（含取消/保存按钮 + ?sheet 唤起脚本），与方案冲突 | 设计师角色清理时只删了「退出登录」，漏掉更早一版的浮层弹窗残留 | 任何「交互模型」级决策（弹窗 vs 就地编辑）落地后，回扫原型是否仍保留旧交互形态：本例删除浮层 CSS + HTML 块 + ?sheet 脚本 + 图例改写，用户卡去掉右侧 chevron（已无下钻语义） | 本文件 §3.15/#42、§3.16 |
| 51 | 2026-07-20 | 换预览二维码后默认头像没了（圆圈只剩绿底）；选照片做头像不居中 | ① `profile.avatarUrl` 存的是上一次预览会话的 `wxfile://` 临时路径，换会话后该路径失效，但 `src="{{avatarUrl || defaultAvatar}}"` 中 avatarUrl 仍为真值覆盖了默认头像（跨预览会话缓存回收的 stale 路径）；② `<image>` 嵌在 `<button open-type="chooseAvatar">` 内受按钮内容流布局干扰，无法控制填满/居中 | ① 已存头像路径必须 `access` 异步校验才回填，失败回落默认并清理存储；② 图片与 chooseAvatar 按钮解耦：`<image>` 作 `.avatar-circle` 直接子元素（`mode="aspectFill"` 居中裁剪），`button` 改为浮在上面的透明触发层（`opacity:0; z-index:2`）——避免按钮布局干扰图片定位 | 本文件 §3.15、§3.17、测试验收报告第八节 |
| 52 | 2026-07-20 | 第三轮修头像后回归：点昵称却拉起换头像浮层（覆盖区太大，最右侧才能改名字） | 绝对定位透明覆盖层 `.avatar-btn` 用 `width:100%;height:100%`，真机上 `%` 宽度**解析到视口 750rpx** 而非 112rpx 头像容器，覆盖层铺满整张卡片盖住右侧昵称（修头像时**未做昵称回归**导致） | ① 头像的 chooseAvatar 覆盖层**必须显式尺寸** `width:112rpx;height:112rpx`（相对 `position:relative` 的 `.avatar-wrap` 定位），禁用 `%`；② 改一个功能点后必须做相关功能回归（昵称/兄弟节点交互），用静态断言（T-W01/T-W02）卡死覆盖层不得外溢盖住昵称；③ 见 §3.18 | 本文件 §3.15、§3.17、§3.18、测试验收报告第九节 |
| 53 | 2026-07-20 | 第四轮显式 `width:112rpx` 仍没解决「点昵称拉起换头像」（绝对定位覆盖层在真机脆） | 绝对定位元素的命中区/包含块解析在微信真机上不可靠，第四轮改显式 112rpx 仍未生效；纯技术精准控制命中区难 | ① **去掉绝对定位覆盖层**——`.avatar-btn` 改为**头像即按钮本身**（普通 flex 子元素，`border-radius:50%+overflow:hidden`，image 嵌内 flex 居中），被 flex 布局死死限制在 112rpx 头像列，**物理不可能外溢盖住昵称**；② **产品方案兜底**（用户拍板）：昵称右侧加独立可点击 ✎ 编辑图标 `.name-edit`（`bindtap` 聚焦输入框），引导点右边改名字；③ 回归纪律不变：改完仍做相关回归，T-W01~T-W04 卡死（禁 position:absolute、昵称右侧有 name-edit）；④ 见 §3.15、§3.18、测试验收报告第十节 |
...
| 57 | 2026-07-20 | v2.0 头像改用 `wx.chooseImage`（album+camera）选图，《隐私保护指引》原选「未采集用户隐私」，但提交审核时平台弹窗已实测检测到隐私接口 **Album**；App 内未弹隐私授权框 | 微信隐私规则（基础库 2.32.3+）：chooseImage/chooseMedia 等涉及相册/摄像头的能力，须后台《隐私保护指引》声明对应 scope 且用户授权弹窗通过后才能调用；否则 enforcement 设备直接失败、提审可能因「未声明却调用」被拒，或发布后权限被回收 | ① 用户已点击提交，平台提示 Album → 必须点「取消」；在公众平台《隐私保护指引》把「是否收集=是」，增加两项：相册(用途=用户设置头像时从相册选图)、摄像头(用途=用户设置头像时拍照)，均填「用户主动操作、仅设置头像时触发」；可选补「头像、昵称」(用途=用户设置个人资料)；保存后指引进审核，再重新提交小程序版本审核；② App 内调 chooseImage 前补 `wx.requirePrivacyAuthorize` 授权弹窗（基础库 2.32.3+ enforcement 设备需要）；③ v1.0 攻略「未采集隐私」对 v2.0 不适用，指引须更新；④ 见 #40/#41、微信小程序提审攻略-2026.md | 本文件 §3.15、§6 待办 41、攻略 v2.0 注 |
| 58 | 2026-07-20(2026-07-23 依上线真值修订) | 统计页日历颜色语义：上线真值=🟢绿色(全完成)/🟠橙色(还没打卡，有任务0完成)/⚪灰色(没任务，无任务)；原#58提出的「黄色=0完成/橙色=部分完成」未落地，且「灰色=还没打卡」与上线矛盾（上线灰色=没任务） | ① 设计稿日历三色必须对齐上线真值（§3.37）；② 上线不区分「0完成」与「部分完成」，均=橙色「还没打卡」；③ 设计稿任何颜色/数据/图标/文案必须以上线小程序现状为准（产品owner=唯一真相源），不能用未落地的修复方案做设计 | §3.37 / 验收报告第十五节 / `ui-mockup-v2.1-guide.html` |
| 59 | 2026-07-20 | 未来日期（如 21/22/23 号）被日历着色（黄/橙/绿），用户指出未来任务不应显示进度色 | `_buildCalGrid` 只按任务/完成状态着色，未判断日期是否在未来；未来日期任务尚未到可打卡时间，显示颜色会误导用户 | 未来日期无论是否有任务/完成记录，统一保持白底（color=''、backfill=false），仅当天及历史日期按方案 B 着色。修复：先算 `isFuture = date > todayDate`，着色逻辑包在 `if (!isFuture)` 内；backfill 用 `let` 可重置；T-C06+T-W04 防回归 | 本文件 §3.15 第九轮、验收报告第十六节、#58 |
| 60 | 2026-07-20 | 日历显示 7月13日 为绿色（全完成），点进去却还有 2 项任务未完成 | 日历 `_buildCalGrid` 用全局未删除任务数作为分母，且完成数直接数 `checkinsByDate[iso]` 的 done 记录，未按当天重复规则过滤；打卡页 `getActiveTasks()` 按 `repeat_type` 算出当天活跃任务，两者口径不一致 | 日历着色必须与打卡页「当天活跃任务」口径一致：逐日按 `dayOfWeek` 过滤 `activeTasks`（daily/weekday/weekend/custom），分母=当天活跃任务数，分子=其中 `ci[t.id].done=true` 的数量；补字也仅看当天活跃任务。新增 T-C07/T-C08 防回归 | 本文件 §3.15 第十轮、验收报告第十七节、#59 |
| 61 | 2026-07-20 | 补卡完成无正向反馈（体验缺口，用户建议加庆祝） | 原 `home.js` 庆祝仅 `isToday(viewDate)` 触发，历史日补卡完成不弹 | 补卡/今天，只要当天最后一个活跃任务完成即弹一次；文案动态：今天=「今天的任务全部完成！」，补卡日=`fmtDateShort(viewDate)+"的任务全部补卡完成！"`；动画/图标复用现有 celebration-overlay；用 `celebrationText` 字段驱动 wxml。新增 `scripts/test_home.js` 防回归 | 本文件 §3.19、验收报告第十八节 |
...
| 66 | 2026-07-22 | v2.1 设计稿 G5 徽章墙 6 个徽章 5 个是编的（「第一棵苗/坚持7天/读完10本/集满100星/连续30天」全错名+错图标），与线上 `BADGE_DEFS` 严重脱节 | 出设计稿时凭印象「凭感觉画」徽章，未对照源码 `miniprogram/utils/constants.js` 的 `BADGE_DEFS` 数组 | **设计稿数据驱动原则**：徽章/任务/图标/名称/进度文案等**任何静态数据**必须从源码常量/枚举读取，禁止凭印象发挥。出稿前对每张图引用的数据执行 grep 对照源文件。真实 `BADGE_DEFS`：🌱 新芽初绽(7星) / 🌿 小有所成(30星) / 🪴 枝繁叶茂(100星) / 🍎 硕果累累(365星) / 🔥 连续打卡(7 streak) / 🌕 满月打卡(30 streak) / 🔒 敬请期待(预留) | `ui-mockup-v2.1-guide.html` / §3.31 |
| 69 | 2026-07-22 | v2.1 设计稿 PNG 自检「通过」但 emoji 显示空白，导致 3 轮 PNG 都被误导（用户反馈「PNG 没改但 HTML 改了」其实就是 PNG 一直都没真正显示 emoji） | ① Chromium headless 不主动用系统装的 emoji 字体；② 我只检了 PNG 尺寸/裁剪/像素颜色，未做「饱和像素检测」验证 emoji 是否真渲染；③ 自检脚本输出「24/25 通过」就以为 OK，实际是布局对了但 emoji 全空白 | 设计稿 PNG 自检必须含：① 饱和像素检测（max(R,G,B)-min(R,G,B)>50）证明有彩色 emoji；② base64 内嵌图 + 字体声明双管齐下（§3.34）；③ emoji 内容多的设计稿优先让用户在浏览器打开 HTML 直接校对（最准） | §3.34 / `ui-mockup-v2.1-guide.html` |
| 70 | 2026-07-22 | v2.1 引导 v3 用户给 5 点反馈——① 站位松鼠挡按钮（G1）/② 气泡离操作点远（G2-G6）/③ 文本里嵌 🐿️ 出现两只松鼠（G5）/④ 跳过与气泡分居顶部和底部→视线上下跳 | v3 把『松鼠角色』『气泡文案』『跳过控件』『操作点锚』四个引导要素当独立组件摆放，违反空间同区原则——任意两两分离都会强制视线跳转 | **引导浮层三要素必须同区**：① 文案（气泡/松鼠说话气泡）与『跳过/退出』必须同一个 UI 单元（如气泡卡片右上『跳过 ✕』），绝不分离到页面顶部；② 气泡必须紧贴当前操作点（小尾巴明确指向），不能下拉到页面底部；③ IP 角色如果保留，唯一来源 = 气泡头像，不再让文本里嵌第二个同款 emoji；④ 高风险操作按钮（如添加）严禁被任何引导浮层元素遮挡。设计稿任何一帧若违反任一条必须返工 | §3.36 / #66 / #67 / #68 / `ui-mockup-v2.1-guide.html` |
...
| 75 | 2026-07-23 | 自检指标把 center-dist 当真问题（高目标被误判 FAIL） | 中心距对扁平/高瘦目标有几何惩罚，未对应视觉诉求 | 自检指标 = 视觉诉求的直接函数（矩形边间隙/覆盖/对齐），不要拿易于写公式的中心距当唯一判据 | verify_guide.py：`bubbleDist`(中心距) → `bubbleGap`(rect_gap)，阈值 ≤ 40px |
...
| 76 | 2026-07-23 | "刚完成首次打卡"状态里累计星星=8、同时新芽初绽（7★门槛）已 earned | 设计"一致态"时只盯着单个数字，没核对其余依赖字段（门槛/earned/未读任务计数） | 同一帧画面所有数据字段必须**相互自洽**：累计星星 vs 徽章 earned vs 连续天数 vs 任务完成计数——改一个就 grep 它的依赖字段全部核一遍 | 5 帧统一 8→1 + 移除 earned |
...
41. 🔜 **日历配色修复（方案 B + 未来日白底 + 活跃任务口径对齐，2026-07-20）**：用户真机发现「有任务0完成」日显示白底，已按方案 B 修复（黄/橙/绿+补字）；后续复核又发现未来日期不应着色，已追加「未来日期统一白底」逻辑；再后续发现 7月13日 日历绿但点进去还有未完成，已修复为「日历分母/分子与打卡页当天活跃任务口径一致」。25/25 自测 ✅、compile ✅、preview 重出 ✅、已 commit+push。`scripts/test_home.js` 已加（补卡完成庆祝弹窗 4/4 ✅，见 §3.19 / #61）。⚠️ 状态更新（2026-07-20 收口）：① ✅ 已 `upload` 2.0.1；② ✅ **已提交审核**（用户操作 2026-07-20，推断已按 #57 声明相册+摄像头后提交，否则平台拦截；审核队列 1-7 天，结果待用户告知）；③ 🔜 代码侧补 `wx.requirePrivacyAuthorize` 隐私授权弹窗（基础库 2.32.3+ enforcement 设备需要），可并入下版用户引导或单独 2.0.2。详见 §3.15 第八/九/十轮 + §3.19 + #57 + #58 + #59 + #60 + #61 + 验收报告第十五/十六/十七/十八节。

42. ✅ **用户引导功能（v2.1）需求已确认 + PRD 已落地**：受众=儿童；首次引导=交互式弱引导(可跳过)；成长页常驻操作手册(纯静态图文)；松鼠果果拟人化引导(情感连接)。框架 `docs/需求框架-用户引导功能.md` v0.3 已确认；PRD `prd-成长小树苗.md`「版本 2.1」章节(EARS: G1–G4 + 核心流程 + 验收 + 边界)已写。🔜 **下一步 = 设计稿（阶段三）**：松鼠引导覆盖层/对话气泡视觉 + 操作手册单页视觉 + 成长页手册图标位置(§3.8 单页不堆独立页)；评审通过后进技术方案→研发。启动条件仍记：2.0.1 审核结果 + 《隐私保护指引》声明审核通过。关联 #57、§3.8、§3.10、§3.15、§3.20。

43. ✅ **新会话环境一键复现（本会话 2026-07-21 新沙箱）**：用户通过 `/root/uploads/<ts>-SESSION-ENV-SETUP.md` 传入环境搭建指南，据此复现 git clone（main，最新提交 `9db48ca`） + `.private/` 凭证(credentials.env/pat.txt/private.key) + miniprogram-ci 安装（1082 包） + compile ✅ + preview 二维码 `.preview-qr.png`（47757 bytes，robot=1）。环境就绪，可继续推进。⚠️ 注意：本新沙箱 `/workspace` 为干净态，`/workspace/MEMORY.md` 软链已不存在；真实记忆文件在仓库根 `/workspace/growth-sapling/MEMORY.md`（本会话已读取）。当前流程位置回顾：v2.0.1 已提审待结果（2026-07-20，审核队列 1-7 天）+ v2.1 用户引导功能 PRD 已落地（下一步=阶段三设计稿）。待用户：① 告知 v2.0.1 审核结果；② （如通过）推进 v2.1 阶段三设计稿；③ 继续真机体验并反馈。

44. ✅ **v2.0.1 审核通过 + v2.1 阶段三设计稿启动（2026-07-21，用户确认）**：用户告知 v2.0.1 审核已通过，并指令「直接以设计师角色做设计稿」。Sophia 切设计师/交互设计师角色，基于 v2.1 PRD(G1–G4)+design-spec，产出高保真视觉稿 `ui-mockup-v2.1-guide.html`（6 帧 + 设计说明，Route A，Token 严格对齐）。核心决策：真实松鼠IP拟人 / 弱引导覆盖层(不锁屏) / G3 官方授权弹窗 / G4 成长页增量新增手册入口。🔜 **待用户评审确认**（§3.2 两步确认：稿未 commit，确认后才 commit+push）→ 评审通过进技术方案→研发。发布状态(v2.0.1 是否点发布)亦待用户告知。**注（2026-07-22）**:用户随后反馈稿中部分区域看不到；根因为头像路径404(`miniprogram-assets/avatar-144.png` 不存在)+引导遮罩 opacity 0.45 过深，已修复为正确路径+0.30 遮罩，Chromium 渲染验证通过。

45. ✅ **v2.1 设计稿渲染问题修复并二次验证（2026-07-22）**：用户反馈稿中「好多地方看不到」；根因① 头像引用 `miniprogram-assets/avatar-144.png` 文件不存在→改为 `miniprogram/assets/avatar-144.png`；② 引导遮罩 `rgba(0,0,0,0.45)` 太深→降至 0.30(授权弹窗背景 0.35)。已用 Chromium headless 整页渲染验证，6 帧均正常显示。修复 commit `6a54495`。🔜 待用户最终确认设计稿 → 技术方案 → 研发。关联 #63。

46. ✅ **v2.1 设计稿 PNG-first 流程采纳 + 图片 base64 修复（2026-07-22）**[已废]
48. 🔄 **v2.1 设计稿第三轮重做（真实弹窗+松鼠说话，2026-07-22）**：用户对第二轮 6 帧稿再给 2 点反馈：① G1 弹窗结构凭印象（#68）→ 按 `tasks.wxml` 1:1 复刻（标题「添加任务」+ X、iconSet 16 项、4 选 1 重复、取消+添加）；② 气泡定位不准（§3.31 规则被否）→ 改用「松鼠说话」方案（§3.32）：松鼠作为独立角色右下角站立 + 气泡发自松鼠、不再指向具体目标。HTML 已重写 + 自检 30/30 ✅ + Chromium 渲染 PNG 待用户最终确认 → 确认后 commit + push。设计稿重写后切架构师角色写 tech-spec v2.1（页面/组件/状态/埋点）。关联 #66/#67/#68、§3.25/§3.26/§3.27/§3.28/§3.29/§3.30/§3.32/§3.33。

47. ✅ **v2.1 设计稿 PNG-first 流程采纳 + 图片 base64 修复（2026-07-22）**：用户明确要求设计稿走「先出 PNG 确认效果 → 再出/定稿 HTML」流程（一步甩完整多帧 HTML 稿易错不易 review）。同时定位「较多缺失」= 初版用相对路径图片(违反 #46)在预览里裂图；已将 6 处图片全部 base64 内嵌，Chromium 渲染验证通过。🔜 后续设计稿一律：base64 内嵌图片 + 出稿前 Chromium 渲染 PNG 自查 + 用户确认 PNG 后再定稿 HTML。关联 #46/#64、§3.21。
50. 🔄 **v2.1 设计稿交付流程改为 HTML 单文件（覆盖 #47 PNG-first，2026-07-22 用户拍板）**：用户指出维护 PNG+HTML 双文件有不一致风险，且 PNG 曾因 #69 emoji 渲染 bug 误导 3 轮评审；HTML 用户可直接浏览器打开校对。决策：设计稿只交付 `ui-mockup-v2.1-guide.html` 单文件（base64 内嵌 + emoji 字体声明），PNG 降为「按需从 HTML 渲染的一次性预览」、不 commit 不手改。§3.21「先 PNG 确认→再 HTML」流程作废，由 §3.35 取代。关联 #46/#69、§3.21/§3.34/§3.35。🔜 待用户浏览器打开 HTML 最终确认 → commit HTML → 切架构师写 tech-spec v2.1。
