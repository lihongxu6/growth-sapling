...
| **v2.1 用户引导功能** | 🔜 需求已确认·PRD 中 | 微信小程序原生 + 本地存储；松鼠 IP 拟人引导 | 交互式首次引导（弱引导/可跳过）+ 隐私透明说明 + 权限授权衔接 + 常驻操作手册（见「版本 2.1」章节） |
...
| 退出登录（推迟至账户体系·云版） | 个人号无需多账号切换；提供“退出登录”按钮清空本地态（入口在「我的成长」页底部）。v2.0 本地版不含，待云开发账户体系上线时实现。 | P1（云版） |
...
> 需求框架：`docs/需求框架-用户引导功能.md`（v0.3，已确认）。本版本在 v2.0 本地版（开屏页 + 成长页用户信息区）基础上，新增「儿童首次交互引导」与「常驻操作手册」，并以松鼠果果拟人化形象贯穿，建立情感连接。范围不含云账户体系（已推迟，§3.14）。

### 2.1.1 背景与目标

**背景**：v2.0 本地版已提交审核。儿童首次进入小程序时，缺少对核心操作（建任务 / 打卡 / 补卡 / 看日历 / 看徽章 / 改头像昵称）的引导，也无数据归属与隐私说明，易产生「不知道从哪开始」「数据去哪了」「为何要相册授权」的疑虑。

**目标**：

| # | 目标 | 衡量 |
|---|---|---|
| 1 | 儿童首次进入即可跟着走完核心操作流程 | 引导覆盖 创建任务 / 打卡 / 补卡 / 看日历 / 看徽章 / 改头像昵称 6 个核心点 |
| 2 | 儿童/家长理解「数据仅存本机、不上传」 | 引导中含隐私说明，且授权弹窗文案一致 |
| 3 | 引导所建任务成为真实账户首个数据 | 引导结束后任务仍存在于账户，不被删 |
| 4 | 随时可回看操作手册 | 成长页有图标入口，点击可见图文手册 |
| 5 | 与产品建立情感连接 | 松鼠果果拟人化贯穿引导与手册 |

### 2.1.2 用户故事

- **儿童（7-8岁）**：我想自己学着用这个小程序，但第一次打开不知道点哪；希望有个可爱的伙伴带我点一遍，做完后我的任务就留在我手机里。
- **家长**：我担心孩子数据上传、也担心他不会用；希望打开就有引导，并清楚说明数据只在本机。

### 2.1.3 功能清单

| 编号 | 功能 | 优先级 |
|---|---|---|
| G1 | 交互式首次引导（弱引导 / 可跳过）：清空数据 → 松鼠引导走 建任务 → 打卡 → 补卡 → 看日历 → 看徽章 → 改头像昵称；所建任务沉淀真实账户 | P0 |
| G2 | 隐私透明说明：引导内嵌「数据仅存本机、不上传」童趣说明 | P0 |
| G3 | 权限授权衔接：chooseImage 前 `wx.requirePrivacyAuthorize` 弹窗，文案与 G2 一致（#57）| P0 |
| G4 | 常驻帮助页 / 操作手册：成长页图标入口 → 纯静态图文手册 | P0 |

### 2.1.4 需求详述（EARS）

**G1 交互式首次引导**
- **[Ubiquitous]** 系统应在用户首次进入且未完成引导时，提供由松鼠果果引导的交互式引导。
- **[Event-driven]** 当用户首次进入小程序（本地无任务数据且 `guideDone=false`）时，系统应清空本地数据（仪式性重置），并启动首次引导。
- **[Event-driven]** 当引导进行到「改头像昵称」步骤且即将调用 `wx.chooseImage` 时，系统应先触发权限授权（见 G3）。
- **[Event-driven]** 当用户点击「跳过」时，系统应标记 `guideDone=true` 并直接进入首页，不再自动引导。
- **[State-driven]** 当引导进行中（弱引导模式），系统应在当前步骤相关 UI 元素上显示高亮提示与松鼠对话气泡，但**不阻止**用户自由操作其他功能、不强制顺序。
- **[State-driven]** 当 `guideDone=true` 时，系统不应再次自动启动引导。
- **[Unwanted]** 若用户在引导中途退出（如杀进程），系统应保留已操作进度（不重复清空、不回滚）；再次进入时若已有任务数据，不再清空，提示「继续引导」或直接开始（见 R1）。
- **[Optional]** Where 用户在中途退出后再次进入且已有部分数据，系统应提供「继续引导 / 跳过」选择，而非强制重走。

**G2 隐私透明说明**
- **[Event-driven]** 当首次引导进行到隐私说明步骤时，系统应通过松鼠气泡以儿童语言说明「你的打卡、头像昵称都只存在这台手机里，不会上传到网上」。
- **[Unwanted]** 若用户/家长对隐私有疑问，系统应在引导与操作手册中均可回看该说明。

**G3 权限授权衔接**
- **[Event-driven]** 当用户在引导或日常使用中触发 `wx.chooseImage`（相册 / 摄像头）前，在基础库 ≥ 2.32.3 的设备上，系统应先调用 `wx.requirePrivacyAuthorize` 弹出隐私授权框，文案与 G2 一致。
- **[Unwanted]** 若用户拒绝授权，系统应回退到「仅相册选择」或提示无法设置头像，不闪退、不阻塞其他功能。

**G4 常驻帮助页 / 操作手册**
- **[Ubiquitous]** 系统应在成长页（我的成长）常驻展示一个操作手册入口图标。
- **[Event-driven]** 当用户点击该图标时，系统应展示纯静态图文操作手册（覆盖核心功能步骤说明），**不触发数据清空**。
- **[Optional]** Where 真实松鼠 IP 已落地（见 §1 / `miniprogram/assets/avatar-144.png`），系统应直接使用真实 IP 呈现引导与手册中的松鼠角色（无占位、无 emoji）。

### 2.1.5 核心流程（引导步骤）

```
开屏页 → [首次进入·无 guideDone]
  1. 松鼠欢迎 + 说明「我们要一起种一棵小树苗」（清空数据）
  2. 引导创建第一个任务（高亮「+ 新建任务」）
  3. 引导完成今天打卡（高亮任务勾选）
  4. 引导补卡一个历史日（高亮日历某日 → 补卡）
  5. 引导看日历（讲解黄/橙/绿含义）
  6. 引导看徽章墙（介绍徽章体系）
  7. 引导改头像昵称（触发相册授权说明 + G3）
  8. 完成：「你的小树苗发芽啦！」所建任务留在账户
[任意步可「跳过」→ 直接进首页]
```

### 2.1.6 交互说明

- **松鼠拟人**：松鼠果果以对话气泡 + 简单动作（招手 / 点头）贯穿引导与手册，语气童趣、鼓励式（「你真棒！」），目的为拟人化、建立情感连接。
- **弱引导**：当前步骤相关元素加高亮光圈 + 松鼠气泡，其余界面可自由操作；不强制顺序、不锁屏。
- **跳过**：引导任意步提供「跳过」入口，点击即 `guideDone=true` 进首页。
- **操作手册**：成长页新增图标（建议与既有图标风格一致，置于用户信息区或页脚），点击打开图文手册（单页，§3.8 不堆独立页）；手册为静态图文，覆盖 建任务 / 打卡 / 补卡 / 日历 / 徽章 / 头像昵称。

### 2.1.7 验收标准

- 首次进入（无 `guideDone`）触发引导，清空数据后儿童可跟着走完 6 个核心步骤；
- 引导所建任务在引导结束后仍存在于账户、未被删；
- 改头像时授权弹窗文案与引导隐私说明一致；
- 成长页有操作手册图标，点击可见图文手册，且**不触发清空**；
- 引导任意步可「跳过」直接进首页；再次进入不自动重引导；
- 不新增独立「教程中心」页（用单页操作手册，§3.8）；
- 现有已上线模块（连续天数 / 完成率 / 月历 / 徽章墙）布局与交互零变化（§3.10）。

### 2.1.8 边界场景

- 中途退出引导：保留进度，不重复清空；再次进入给「继续 / 跳过」。
- 拒绝相册授权：回退仅相册或提示无法设头像，不闪退。
- 已有数据的新装 / 清缓存后：若 `guideDone` 被清但已有任务，进入不重复清空，提示继续或跳过。
- 低版本基础库（< 2.32.3）：G3 授权弹窗不可用，降级为直接调 chooseImage（与现状一致），隐私说明仍由 G2 覆盖。

### 2.1.9 数据口径 / 埋点（建议）

- 新增本地标记 `guideDone`（bool，存 `wx.setStorageSync`）。
- 建议埋点：引导启动次数、每步完成率、跳过率、操作手册打开次数（衡量引导效果，供上线复盘）。

### 2.1.10 待确认问题

- 无（框架 v0.3 已确认全部分叉与松鼠拟人决策）。
